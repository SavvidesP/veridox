...
import { ArrowRight, CheckCircle, XCircle, AlertTriangle, Trash2, TrendingUp, TrendingDown, Lightbulb, Check } from 'lucide-react';
...
export default function Cascading() {
  ...
  async function fetchData() {
    ...
    // Calculate PSP stats from transactions
    const stats = {};
    (txns || []).forEach(t => {
  ...
  function generateSuggestions(rules, stats, psps) {
    ...
    const bestPsp = Object.entries(stats)
      .filter(([_, s]) => s.total >= 3)
      .sort((a, b) => parseFloat(b[1].approvalRate) - parseFloat(a[1].approvalRate))[0];
    ...
    rules.forEach(rule => {
      ...
      // Suggestion 1: Low approval rate on primary PSP
      if (approvalRate < 50 && bestPsp && bestPsp[0] !== rule.psp.name) {
        suggestions.push({
          id: `low-approval-${rule.id}`,
          ruleId: rule.id,
          type: 'warning',
          title: `Low approval rate on "${rule.name}"`,
          description: `${rule.psp.name} has only ${approvalRate}% approval rate. Consider switching to ${bestPsp[0]} (${bestPsp[1].approvalRate}% approval rate).`,
          action: 'swap_primary',
          currentPsp: rule.psp.name,
          suggestedPsp: bestPsp[0],
          suggestedPspId: psps.find(p => p.name === bestPsp[0])?.id,
          ruleData: rule,
        });
      }

      // Suggestion 2: No fallback PSP set
      if (!rule.fallback && psps.length > 1) {
        const fallbackCandidate = psps.find(p => p.name !== rule.psp.name && p.status === 'active');
        if (fallbackCandidate) {
          suggestions.push({
            id: `no-fallback-${rule.id}`,
            ruleId: rule.id,
            type: 'info',
            title: `No fallback PSP for "${rule.name}"`,
            description: `Rule has no fallback. Adding ${fallbackCandidate.name} as fallback will improve resilience.`,
            action: 'add_fallback',
            suggestedPsp: fallbackCandidate.name,
            suggestedPspId: fallbackCandidate.id,
            ruleData: rule,
          });
        }
      }

      // Suggestion 3: Fallback PSP performing better than primary
      if (rule.fallback) {
        const fallbackStats = stats[rule.fallback.name];
        if (fallbackStats && parseFloat(fallbackStats.approvalRate) > approvalRate + 15) {
          suggestions.push({
            id: `swap-fallback-${rule.id}`,
            ruleId: rule.id,
            type: 'warning',
            title: `Swap PSPs for "${rule.name}"`,
            description: `Fallback ${rule.fallback.name} (${fallbackStats.approvalRate}%) is outperforming primary ${rule.psp.name} (${approvalRate}%). Consider swapping them.`,
            action: 'swap_primary_fallback',
            currentPsp: rule.psp.name,
            currentPspId: rule.psp_id,
            suggestedPsp: rule.fallback.name,
            suggestedPspId: rule.fallback_psp_id,
            ruleData: rule,
          });
        }
      }
  ...
  async function applySuggestion(suggestion) {
    ...
    if (suggestion.action === 'swap_primary' || suggestion.action === 'swap_primary_fallback') {
      await supabase.from('routing_rules').update({
        psp_id: suggestion.suggestedPspId,
        fallback_psp_id: suggestion.action === 'swap_primary_fallback' ? rule.psp_id : rule.fallback_psp_id || null,
      }).eq('id', rule.id);
    } else if (suggestion.action === 'add_fallback') {
      await supabase.from('routing_rules').update({
        fallback_psp_id: suggestion.suggestedPspId,
      }).eq('id', rule.id);
    }
  ...
  return (
    <div style={{ padding: '32px', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: '#64748B', fontSize: '13px', margin: '4px 0 0' }}>Visual PSP fallback flow with smart optimization suggestions</p>
      </div>
          ...
          {filteredRules.map(rule => (
            <div key={rule.id} style={{ background: 'white', borderRadius: '14px', border: '1px solid #E2E8F0', padding: '20px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '11px', color: '#94A3B8', marginTop: '2px' }}>
                      {[rule.brand && `Brand: ${rule.brand}`, rule.currency && `Currency: ${rule.currency}`, rule.type && `Type: ${rule.type}`, rule.country && `Country: ${rule.country}`].filter(Boolean).join(' · ') || 'No conditions set'}
                    </div>
                  </div>
                </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                ...
                {rule.psp ? (
                  <div style={{ background: '#EEF2FF', border: '2px solid #6366F1', borderRadius: '10px', padding: '12px 16px', textAlign: 'center', minWidth: '130px' }}>
                    <div style={{ fontSize: '11px', fontWeight: '700', color: '#94A3B8', marginBottom: '4px' }}>PRIMARY PSP</div>
                    <div style={{ fontSize: '14px', fontWeight: '700', color: '#4338CA' }}>{rule.psp.name}</div>
                    {pspStats[rule.psp.name] && (
                      <div style={{ fontSize: '11px', marginTop: '4px', fontWeight: '600', color: parseFloat(pspStats[rule.psp.name].approvalRate) >= 70 ? '#166534' : '#991B1B' }}>
                        {pspStats[rule.psp.name].approvalRate}% approval
                      </div>
                    )}
                    <CheckCircle size={14} color="#22C55E" style={{ marginTop: '4px' }} />
                  </div>
                ) : (
                  <div style={{ background: '#F8FAFC', border: '2px dashed #E2E8F0', borderRadius: '10px', padding: '12px 16px', textAlign: 'center', minWidth: '130px' }}>
                ...
                {rule.fallback && (
                  <>
                    ...
                    <div style={{ background: '#FFF7ED', border: '2px solid #F59E0B', borderRadius: '10px', padding: '12px 16px', textAlign: 'center', minWidth: '130px' }}>
                      ...
                      {pspStats[rule.fallback.name] && (
                        <div style={{ fontSize: '11px', marginTop: '4px', fontWeight: '600', color: parseFloat(pspStats[rule.fallback.name].approvalRate) >= 70 ? '#166534' : '#991B1B' }}>
                          {pspStats[rule.fallback.name].approvalRate}% approval
                        </div>
                      )}
                      <CheckCircle size={14} color="#22C55E" style={{ marginTop: '4px' }} />
                    </div>
