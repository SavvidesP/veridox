...
export default function AddClient() {
  ...
  const [form, setForm] = useState({
    ...
    company: '', country: '', industry: 'Forex', dob: '', assignedTo: 'Maria K.',
  });
      ...
      <div style={{ maxWidth: '640px' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #E2E8F0', padding: '24px', marginBottom: '24px' }}>
            <div style={{ color: '#0F172A', fontSize: '13px', fontWeight: '700', marginBottom: '16px', paddingBottom: '12px', borderBottom: '1px solid #F1F5F9' }}>Assignment</div>
            <div style={{ maxWidth: '300px' }}>
              <label style={labelStyle}>Assigned Compliance Agent</label>
              <select
                value={form.assignedTo}
                onChange={e => setForm({ ...form, assignedTo: e.target.value })}
                style={{ ...inputStyle, cursor: 'pointer' }}
              >
                <option>Maria K.</option>
                <option>Nikos P.</option>
                <option>Anna S.</option>
              </select>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            ...
            <button type="button" onClick={() => navigate('/clients')} style={{
