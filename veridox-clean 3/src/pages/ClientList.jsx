...
const kycBadge = (status) => {
  ...
  return (
    <span style={{ ...styles[status], padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '600' }}>
      {labels[status]}
    </span>
  );
};
...
export default function ClientList() {
  ...
  const filtered = clients.filter(c => {
    ...
    const matchFilter = activeFilter === 'all' || c.kycStatus === activeFilter;
    ...
    <div style={{ padding: '32px', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          ...
          <p style={{ color: '#64748B', fontSize: '13px', margin: '4px 0 0' }}>{clients.length} total clients across all industries</p>
        </div>
        <button
        ...
        {filters.map(f => (
          <button
            ...
            style={{
              ...
              fontSize: '12px',
              fontWeight: '600',
              background: activeFilter === f.key ? '#6366F1' : 'transparent',
          ...
          >
            ...
            {f.key !== 'all' && (
              <span style={{ marginLeft: '6px', opacity: 0.7 }}>
                {clients.filter(c => c.kycStatus === f.key).length}
              </span>
            )}
          </button>
        ))}
      ...
      <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #E2E8F0', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#F8FAFC', borderBottom: '1px solid #E2E8F0' }}>
              {['Client', 'Company', 'Country', 'Industry', 'KYC Status', 'Agent', 'Added', ''].map(h => (
          ...
          <tbody>
            {filtered.map(client => (
              <tr
                ...
                style={{ borderTop: '1px solid #F1F5F9', cursor: 'pointer', background: 'white', transition: 'background 0.1s' }}
              ...
              >
                <td style={{ padding: '14px 20px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div>
                      <div style={{ color: '#0F172A', fontSize: '13px', fontWeight: '600', whiteSpace: 'nowrap' }}>{client.name}</div>
                      <div style={{ color: '#94A3B8', fontSize: '11px' }}>{client.email}</div>
                    </div>
                  </div>
                </td>
                <td style={{ padding: '14px 20px', color: '#475569', fontSize: '13px', whiteSpace: 'nowrap' }}>{client.company}</td>
                <td style={{ padding: '14px 20px', color: '#475569', fontSize: '13px' }}>{client.country}</td>
                <td style={{ padding: '14px 20px' }}>
                  <span style={{
                    padding: '3px 8px', borderRadius: '6px', fontSize: '11px', fontWeight: '600',
                    background: client.industry === 'Forex' ? '#EEF2FF' : client.industry === 'Payments' ? '#F5F3FF' : '#FFF7ED',
                    color: client.industry === 'Forex' ? '#4338CA' : client.industry === 'Payments' ? '#7C3AED' : '#C2410C',
                ...
                <td style={{ padding: '14px 20px', color: '#94A3B8', fontSize: '12px', whiteSpace: 'nowrap' }}>{client.createdAt}</td>
        ...
        {filtered.length === 0 && (
          <div style={{ padding: '48px', textAlign: 'center', color: '#94A3B8', fontSize: '13px' }}>No clients found.</div>
        )}
      </div>
    </div>
  );
